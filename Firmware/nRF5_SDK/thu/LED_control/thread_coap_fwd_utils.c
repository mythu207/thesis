                                            ...
                                            int16_t 		length)
{
    ...
    {
      /*
      if(otIp6IsAddressUnspecified(&peer_addr_string))
      {
        NRF_LOG_INFO("Failed to send CoAP Request to the unspecified IPv6 address");
        break;
      }
      */
      ...
      error = otIp6AddressFromString((char *)peer_addr_string, &message_info.mPeerAddr);
      ASSERT(error == OT_ERROR_NONE);
